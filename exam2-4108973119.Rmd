...
##str(data228)
regex1 <- "[:alnum:]+(?=/)"
pick1 <- str_extract(data228$`檔號`,regex1)
regex2<-"(?<=/)[0-9]{4}(?=/)"
pick2<-str_extract(data228$`檔號`,regex2)
data228[[5]]<-list(`全宗名編號`=pick1)
data228[[6]]<-list(`年`=pick2)
data228[c("全宗名編號","年")]
...
str(courseStructure)
regex_a <- "[\u4E00-\u9FFF]+系[1-4]{1}"
majorGradeCombos<-str_extract(courseStructure$`應修系級`,regex_a)
majorGradeCombos
```
...
# econ101junior
str(courseStructure)
pick_a <- courseStructure$`學年`=="103"
pick_b <- courseStructure$`應修系級`=="經濟系3A"
pick_c <- courseStructure$`應修系級`=="經濟系3B"
econ101junior <- courseStructure$`科目名稱`[pick_a&pick_b&pick_c]
econ101junior
```
